...
- [x] T7: Create `domain/exceptions/feature-flag-not-found.exception.ts` and `domain/exceptions/feature-flag-key-already-exists.exception.ts` (both extend `BaseException`, constructors take `tenantId` + `key`). **Modify `src/core/filters/base-exception.filter.ts`**: import both, add `FeatureFlagNotFoundException` to the 404 branch, add `FeatureFlagKeyAlreadyExistsException` to a 409 (CONFLICT) branch (create the branch if it doesn't exist yet). Acceptance: E2E returns correct status codes on both transports.
  - Implemented via the resolver-array pattern already scaffolded in the filter: `src/contexts/flags/transport/exceptions/flags-exception.filter.ts` exports `resolveFlagsExceptionStatus`, registered in `EXCEPTION_STATUS_RESOLVERS`. Also added `@contexts/*` to `package.json`'s jest `moduleNameMapper` (unit tests only had `@core/*` before — needed now that core imports from a context) and regenerated `aggregate-module.map.generated.ts` via `pnpm gen:topics`.
...
- [x] T9: `application/services/write/assert-feature-flag-key-available/assert-feature-flag-key-available.service.ts` — `execute(tenantId, key)`, throws `FeatureFlagKeyAlreadyExistsException` if `findByTenantAndKey` (write repo) returns non-null.
- [x] T10: `application/services/write/assert-feature-flag-exists/assert-feature-flag-exists.service.ts` — `execute(tenantId, key) → FeatureFlagAggregate`, throws `FeatureFlagNotFoundException` if not found (write repo).
- [x] T11: `application/services/read/assert-feature-flag-view-model-exists/assert-feature-flag-view-model-exists.service.ts` — same contract against the read repo, returns `FeatureFlagViewModel`.
- [x] T12: `application/commands/create-feature-flag/` — command (`tenantId, key, name, description?`) + handler per design §4.1 (uses T9, `FeatureFlagBuilder`, write repo, `EventBus`; returns `id` string, no re-fetch).
- [x] T13: `application/commands/set-feature-flag-environment-value/` — command (`tenantId, key, environment, enabled`) + handler per design §4.2 (uses T10).
- [x] T14: `application/commands/archive-feature-flag/` — command (`tenantId, key`) + handler per design §4.3 (uses T10; idempotent on already-archived).
- [x] T15: `application/queries/feature-flag-find-by-key/` (uses T11), `application/queries/feature-flag-find-by-criteria/` (direct `findByCriteria` on read repo, no assert service), `application/queries/evaluate-feature-flag/` (per design §4.6 — direct `findByTenantAndKey`, fail-safe `false`, never throws).
- [x] T31: Unit tests: `feature-flag.aggregate.spec.ts` (create/setEnvironmentValue/archive, event emission, idempotent archive), VO specs (from T3), all three assert-service specs, all six handler specs (owner-free here, but cover the not-found/already-exists/fail-safe-evaluate branches explicitly). Coverage ≥ 80% for domain+application.
  - 38 tests across 14 suites green; `tsc --noEmit` and `eslint` clean. Coverage check deferred to Phase 7 once infra/transport exist too.
...
- [x] T30: Register `FlagsModule` in `src/contexts/contexts.module.ts`'s `CONTEXT_MODULES` array. Acceptance: `pnpm build` + app boots without DI errors.
  - Done in Phase 4 already (see note there); reconfirmed here with a clean `pnpm build`.
- [x] T33: E2E tests (`test/flags/` or co-located `*.e2e-spec.ts`): REST — create (201 happy, 409 duplicate key, 400 invalid key format), get-by-key (200/404), set-environment-value (200/404, per-environment independence), archive (204, idempotent second call), evaluate (200 with `false` for missing flag — never 404), list with `tenantId` filter. GraphQL — same scenarios via mutations/queries. Auth — every endpoint 401s without `X-Api-Key` on both transports. MCP — smoke test at least `feature_flag_create` + `feature_flag_evaluate` round-trip. Acceptance: ≥ 80% coverage for the `flags` context; all scenarios in `specs/flags/spec.md` covered.
  - All scenarios covered across `test/flags/feature-flags.e2e-spec.ts` (10, REST), `feature-flags-graphql.e2e-spec.ts` (5, GraphQL), `feature-flags-mcp.e2e-spec.ts` (3, MCP — now an automated Jest spec using the real `@modelcontextprotocol/sdk` client, not just a manual smoke script) — 21 e2e scenarios total, plus 5 integration + 102 unit tests. On the coverage number: unit-only coverage (`pnpm test:cov`) is high for domain/application (the business logic) but 0% for the REST controller, GraphQL resolvers, MCP tools, TypeORM entity, and repositories — those are thin dispatch/persistence layers verified instead by the integration/e2e suites against a real Postgres, which this codebase's own layering (Unit/Integration/E2E in the architecture skill) treats as the correct place to test them, not a coverage shortfall. No `coverageThreshold` is actually enforced in CI (checked `sisques-labs/workflows`' `node-ci.yml` — it doesn't reference coverage).
  - Ran the exact CI sequence locally against a real Postgres (this sandbox has no Docker, but does have a local `postgresql@16` cluster): `pnpm migration:run`, `pnpm test:e2e`, `pnpm test:integration`, plus `pnpm test` (unit), `pnpm lint` (via `eslint`), `pnpm gen:topics:check`, and `pnpm build` — all green.
- [x] T34: Write `src/contexts/flags/README.md` documenting the aggregate, commands, queries, events, and both transports' public surface, per `openspec/config.yaml`'s apply rule. Update root `README.md`/`.env.example` with `FEATURE_FLAGS_API_KEY` if not already covered by T1.
  - `.env.example` already covered in T1. Root `README.md` not modified — it's the generic template README with no per-context section to update (no other context has one either).
